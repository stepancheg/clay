...
llvm::StringRef typeToStaticStringLiteral(Type* type) {
...
static MultiPValuePtr analyzeExpr3(ExprPtr expr, EnvPtr env)
{
    ...
    case STRING_LITERAL : {
        ...
        v->as<StringLiteralRepr>() = StringLiteralRepr::get(x->value->str);
    ...
    for (size_t i = 0; i < attrs->size(); ++i) {
        ...
        case VALUE_HOLDER : {
            ...
            if (vh->type->typeKind == STRING_LITERAL_TYPE) {
                x->attrAsmLabel = vh->as<StringLiteralRepr>().stringRef();
                break;
            }
            // fallthrough
        }
